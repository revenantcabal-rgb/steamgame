 ...
 * Marketplace Engine
 *
 * BDO-inspired pricing:
 * - Each item has a base price determined by its type/tier/rarity
 * - Tradable range: base ±10%
 * - Prices adjust with supply/demand
 * - 5% tax on sales
 */

import { RESOURCES } from '../config/resources';
import { GEAR_TEMPLATES } from '../config/gear';
import { MARKET_TAX_RATE, PRICE_RANGE_PERCENT } from '../types/marketplace';
import type { MarketPriceInfo, MarketCategory } from '../types/marketplace';

/** Get the developer-set base price for an item */
export function getBasePrice(itemId: string, category: MarketCategory): number {
  switch (category) {
    case 'resources': {
    ...
    case 'accessories': {
      ...
      return tierMultiplier[template.tier] || 100;
    }
    case 'consumables':
...
export function getDevCaps(itemId: string, category: MarketCategory): { floor: number; ceiling: number } {
  const base = getBasePrice(itemId, category);
  return {
...
/** Adjust base price after a sale (supply/demand dynamics) */
export function adjustBasePrice(
  ...
  // If sold at max: base price rises by 1%
  if (salePrice >= range.max * 0.98) {
    return Math.min(devCeiling, Math.floor(currentBase * 1.01));
  }
  // If sold at min: base price drops by 1%
  if (salePrice <= range.min * 1.02) {
    return Math.max(devFloor, Math.floor(currentBase * 0.99));
  }
  return currentBase;
}
